...
```bash
docker run  --name postgres_de_course_2 -it \
  ...
  postgres:13
```
The command `--platform linux/amd64` is to solve the warning:
...
that appears when we use a MacBook Pro M1 
...
If we already have the file in our computer we can run in the terminal:
1. `python3 -m http.server` 
2. `URL=http://(IP computer )/yellow_tripdata_2021-01.csv`
...
`jupyter nbconvert --to=script upload_data.ipynb` OR
